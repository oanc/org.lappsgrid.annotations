...
mvn -DskipTests=true package javadoc:jar source:jar deploy -Dgpg.passphrase="$PGP_PASSPHRASE" --settings settings.xml -Pstaging

# 4. Build and deploy the Maven site to the gh_pages branch.
# The GitHub Maven plugin seems to be having problems with invalid bytecode versions.
